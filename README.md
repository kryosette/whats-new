...
[Content structure preserved - RPKI Validator section]
...
[Content structure preserved - Local Great Analyzer section]
...
MITM  TCP session hijacking 🔴 Critical
  TCP desynchronization 🟠 High
DoS/DDoS  SYN flood 🔴 Critical
  ACK flood 🟠 High
  RST flood 🟠 High
  TCP window size attack 🟡 Medium
  FIN/PSH/URG flood 🟡 Medium
RCE & Kernel Panic  SACK panic (Linux < 4.15) 🔴 Critical
  TCP fragmentation 🟠 High
  MSS (maximum segment size) 🟡 Medium
  TCP timestamp crash 🔴 Critical
Protocol  TCP sequence prediction 🟠 High
  Blind reset attack 🟠 High
  ACK storm 🟡 Medium
State Exhaustion  TCP connection flood 🔴 Critical
  TCP half-open/closed attacks 🟠 High
Options  TCP timestamp exploit 🟡 Medium
  SACK retransmission attack 🟡 Medium
